...
export default function NewsSwiper({
  ...
  return (
    ...
    >
        ...
        {news.map((post) => (
          ...
          >
            ...
            <div className="flex h-full flex-col justify-evenly text-left">
              <h5 className="text-agua-primary-blue wrap-break-words mb-2 line-clamp-2 px-1 text-base md:text-lg">
                {post.title}
              </h5>
      ...
      {/* Navigation Buttons */}
      <button
        className="news-prev-btn absolute top-1/2 -left-3 -translate-y-1/2 md:-left-2 lg:-left-5"
        aria-label="Anterior"
      >
        <ImageOptimized
          src={arrowPrev}
          ...
          layout="fixed"
          loading="lazy"
          className="size-5! transition-transform duration-300 hover:scale-110 md:size-7!"
        />
      </button>
      <button
        className="news-next-btn absolute top-1/2 -right-3 -translate-y-1/2 md:-right-2 lg:-right-5"
        aria-label="Próximo"
      >
        <ImageOptimized
          src={arrowNext}
